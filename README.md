# Golang HashTable Employe

散列表(Hash Table，也叫哈希表)，是根據關鍵碼值(key value)而直接進行訪問的數據結構。
它通過把關鍵碼值映射到表中的一個位置來訪問記錄，以加快查找的整度，這個映射函數叫作散列函數，存放記錄的數據叫作散列表。

# 使用哈希表完成簡易的雇員管理系統

> 要求

1. 不使用數據庫，盡量節省內存，速度愈快愈好 => 哈希表(散列)
2. 添加時，保證按照雇員的ID從低到高插入

> 思路分析

1. 使用鏈表來實現哈希表，該鏈表不帶表頭(即鏈表的第一個節點就存放雇員信息)
2. 代碼實現 增、刪、改、查(顯示所有員工)，按ID查詢。(刪 未完成)

![image](./images/20201213201417.png)

# 執行結果

![image](./images/20201213210625.png)

![image](./images/20201213210658.png)
